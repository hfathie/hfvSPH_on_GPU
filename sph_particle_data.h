...
struct sph_particle_data : public sph_particle_data_hydrocore
{
  ...
  {
    MyFloat unitVelocity = 43672.48;
    MyFloat c_0 = 35063.4;
    MyFloat c_s = 0.2e5;
    MyFloat rho_0 = 2.72e-20; /* g/cm^3 */
    MyFloat rho_1 = 1.0e-14; /* g/cm^3 */
    
    MyFloat unitDensity = 5.9e-20; /* g/cm^3 */
    MyFloat DensityX = Density * unitDensity;
    
    if(DensityX <= rho_0)
        return c_0 / unitVelocity;
    
    if(DensityX > rho_0)
        return sqrt(((c_0*c_0-c_s*c_s)*pow(DensityX/rho_0, -2.0/3.0) + c_s*c_s) * sqrt(1.0 + pow(DensityX/rho_1, 4.0/3.0))) / unitVelocity;
  ...
  inline MyFloat get_pressure(void)
  {
    ...
    MyFloat unitDensity = 5.9e-20; /* g/cm^3 */
    MyFloat unitPressure = 1.1e-10;
    MyFloat DensityX = Density * unitDensity;
    
    if(DensityX <= rho_0)
        return DensityX * c_0*c_0 / unitPressure;
    
    if(DensityX > rho_0)
        return DensityX * ((c_0*c_0-c_s*c_s)*pow(DensityX/rho_0, -2.0/3.0) + c_s*c_s) * sqrt(1.0 + pow(DensityX/rho_1, 4.0/3.0)) / unitPressure;
